...
def alertUnreactedView(request):
    ...
    closedQuery = userOrder.filter(statusCheck='Closed').filter(
        Date_Regularised__gte=datetime.date.today())
    ...
    pendingQuery_today = userOrder.filter(statusCheck='Pending').filter(
        Date_Regularised__gte=datetime.date.today())
    ...
    treated = userOrder.filter(CAMT_Reveiewer=request.user).filter(
        Date_Regularised__gte=datetime.date.today())
    total_treated = treated.count()

    queryset = Alert.objects.exclude(
        statusCheck__in=['Pending', 'Closed']).exclude(role='Branch')
...
def alertPending(request):
    ...
    userOrder = Alert.objects.filter(CAMT_Reveiewer=request.user)
    pendingQuery = userOrder.filter(
        statusCheck='Pending').order_by('Date_Regularised')
...
def alertClosed_staff(request):
    userOrder = Alert.objects.filter(CAMT_Reveiewer=request.user)
    closedQuery = Alert.objects.filter(camtDecision='TruePositive').filter(statusCheck='Closed').filter(
        role='camt').filter(Date_Regularised__gte=datetime.date.today()).order_by('Date_Regularised')
    ...
    #total_closed_query = closedQuery.count()
    userOrder = Alert.objects.filter(CAMT_Reveiewer=request.user)
    closedQuery_count = userOrder.filter(statusCheck='Closed').filter(
        Date_Regularised__gte=datetime.date.today())
...
def alertClosed_user(request):
    userOrder = Alert.objects.filter(CAMT_Reveiewer=request.user)
    closedQuery = userOrder.filter(statusCheck='Closed').filter(
        camtDecision='TruePositive').filter(Date_Regularised__gte=datetime.date.today())
    #userOrder = Alert.objects.filter(CAMT_Reveiewer=request.user)
...
class SearchListView(ListView):
    ...
    def get_queryset(self, *args, **kwargs):
        qs = Alert.objects.filter(camtDecision='TruePositive').filter(
            statusCheck='Closed').order_by('-Date_Regularised')
